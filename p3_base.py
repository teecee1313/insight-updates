...
class BaseConnector:
    ...
    def get_quotes(self, account_id: str, tickers: list[str]) -> dict:
        """ONE batched call: {TICKER: {'last': float|None, 'bid': float|None}}.
        The watcher's only per-cycle data request."""
        raise NotImplementedError

    def place_sell(self, account_id: str, ticker: str, qty: int,
                   limit: float | None = None) -> dict:
        """Fire the exit: Market sell (limit=None) or Day Limit sell."""
        raise NotImplementedError
